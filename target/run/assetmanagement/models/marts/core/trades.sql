      ...
      create or replace transient table finservam.dbt_awong.trades  as
      (
select
              c.*,
         ...
         (
            ...
            from finservam.dbt_awong.stg_stock_history h
            inner join finservam.dbt_awong.watchlist w on h.symbol = w.symbol and w.trader = 'all_authorized'
            where h.close <> 0 and year(date) between 2010 and 2019
         ) c
         full outer join finservam.dbt_awong.traders t
       union all
       ...
       union all
          ...
          from finservam.dbt_awong.stg_stock_history h
          inner join finservam.dbt_awong.watchlist w on h.symbol = w.symbol and w.trader = 'charles'
